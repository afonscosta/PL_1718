	...
	for (pers_name in personagens) {
		...
		for (pers_name_dataset in dataset) {
			...
			if (pers_name_dataset ~ pers_name && 
				...
				print personagens[pers_name] "," pers_name "," dataset[pers_name_dataset] > "pers_bio_freq.csv";	
				encontrou = 1;
			}
		}
		# Se não encontrou nenhuma BIO então coloca sem nada
		if (encontrou == 0) {
			print personagens[pers_name] "," pers_name > "pers_bio_freq.csv";
		}
	}

	#Ordena o ficheiro e apaga o original
	system("sort -rn pers_bio_freq.csv > pers_bio_sort_freq.csv && rm pers_bio_freq.csv");
}
